...
### Rules we did not write: Grafana Asserts (#43)

The stack carries **642** alert and recording rules. **28 of them are ours** — `Monitoring`
(18) and `Apps` (10), the two tables above. Every other one is generated by Grafana Cloud's
**Asserts** app (`grafana-asserts-app`, shown in the UI as "Knowledge graph"), which writes
~600 rules into the `knowledge-graph` folder and ~14 into `frontend-observability-asserts`.
`grafana-apply.py` neither created them nor can reconcile them, and nothing in this repository
reads them: our only two dashboards are `Monitoring/Endpoints` and `Monitoring/Hosts`, and no
rule, dashboard or script here references Asserts.

**What was wrong.** One rule, `asserts_up` in the `AssertsHealth` group, sat firing from
2026-09-16 to 2026-09-20 with no summary, no runbook and no owner — only a `__value_string__`
of `value=1`. It was never going to stop: its query is the literal expression `1`, so it is a
heartbeat that Asserts implements *as a permanently firing `info` alert*. A red line that is
always there is a red line nobody reads, which is how a real alert gets skipped.

**What we did, and why it is not "disable the plugin".** The decision was to stop using Asserts
(option 1 in #43 — we do not use it, and its rules cost active series). That turned out not to be
reachable from here:

- `POST /api/plugins/grafana-asserts-app/settings` with `enabled: false` returns
  **`Cannot disable auto-enabled plugin`**. The plugin is `autoEnabled: true` — Grafana Cloud
  forces it on for the stack, and no stack-level token can turn it off.
- Uninstalling it is a **grafana.com Cloud API** operation, not a stack one. `.env` holds only
  `GRAFANA_SA_TOKEN` (a stack service-account token) and `GRAFANA_SM_TOKEN`; there is no
  grafana.com Cloud Access Policy token, so the only remaining path is the Grafana Cloud portal
  by hand.

So the `AssertsHealth` **rule group was deleted** instead, on 2026-09-20. The other ~613 Asserts
rules were left alone: none of them fire, so none of them pollute the alert list, and deleting
them buys nothing while the plugin that regenerates them is still on.

> ⚠️ **Deleting a plugin-generated rule group needs a different API.** The normal provisioning
> route, `DELETE /api/v1/provisioning/folder/<uid>/rule-groups/<group>`, fails **409
> `provenanceMismatch`**: these rules carry provenance `classic-converted-prometheus`, not
> `classic-api-provisioning`. The one that works is the Prometheus-conversion API, keyed on the
> folder **title**, not its uid:
>
> ```bash
> curl -X DELETE -H "Authorization: Bearer $GRAFANA_SA_TOKEN" \
>   "$GRAFANA_URL/api/convert/prometheus/config/v1/rules/knowledge-graph/AssertsHealth"
> ```

**If `asserts_up` comes back**, the plugin has regenerated it, and the delete above is a
treatment rather than a cure. The cure is then to uninstall `grafana-asserts-app` from the
Grafana Cloud portal (Administration → Plugins, or grafana.com → the stack → Plugins). Re-run
the delete to buy time, and do not silence it open-endedly — a silence with no expiry is the
same permanent-red-line problem wearing a different hat.

**How to check the list is still clean.** `firing: NONE` is the whole test — and it is the one
assertion that makes "something is red" mean something again:

```bash
curl -s -H "Authorization: Bearer $GRAFANA_SA_TOKEN" \
  "$GRAFANA_URL/api/prometheus/grafana/api/v1/rules" \
| python -c "import sys,json;d=json.load(sys.stdin);print([g['file']+'/'+r['name'] for g in d['data']['groups'] for r in g.get('rules',[]) if r.get('state')=='firing'] or 'firing: NONE')"
```
